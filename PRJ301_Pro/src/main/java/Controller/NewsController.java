...
public class NewsController extends HttpServlet {
    ...
    protected void manageNews(HttpServletRequest request, HttpServletResponse response)
        ...
        while (attempt < maxRetries) {
            try {
                ...
                if (attempt == maxRetries) {
                    request.setAttribute("error", "Failed to load news after " + maxRetries + " attempts: " + e.getMessage());
                    break;
                }
        ...
        while (attempt < maxRetries) {
            try {
                ...
                if (attempt == maxRetries) {
                    request.setAttribute("error", "Failed to count news after " + maxRetries + " attempts: " + e.getMessage());
                    break;
                }
                nDB = new TinTucDAO();
                try {
    ...
    protected void addNews(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            int subcategory = subcategoryStr != null && !subcategoryStr.isEmpty() ? Integer.parseInt(subcategoryStr) : 0;
            int classification = classificationStr != null && !classificationStr.isEmpty() ? Integer.parseInt(classificationStr) : 0;
            int adminId = adminIdStr != null && !adminIdStr.isEmpty() ? Integer.parseInt(adminIdStr) : 0;

            TinTuc n = new TinTuc(0, title, image, excerpt, content, null, views, tags, category, subcategory, classification, adminId);
    ...
    protected void editNews(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            TinTuc n = new TinTuc(id, title, image, excerpt, content, null, views, tags, category, subcategory, classification, adminId);

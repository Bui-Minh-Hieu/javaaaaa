    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (maTinTuc == null || maTinTuc.trim().isEmpty()) {
            ...
            try {
                ...
                String updateSql = "UPDATE TinTuc SET SoLanDoc = SoLanDoc + 1 WHERE MaTinTuc = ? AND MaTheLoai = ? AND MaTheLoaiTin = ?";
                try (PreparedStatement updateStmt = conn.prepareStatement(updateSql)) {
                    updateStmt.setInt(1, Integer.parseInt(maTinTuc));
                    updateStmt.setInt(2, 5); // MaTheLoai = 5 (TUYỂN SINH)
                    updateStmt.setInt(3, 13); // MaTheLoaiTin = 13 (SAU ĐẠI HỌC)

        ...
        <c:forEach var="club" items="${clubList}">
            <div class="row mb-4 p-3 bg-light rounded shadow-sm">
                <c:if test="${not empty club.urlAnh}">
                    <div class="col-md-4">
                        <img src="${club.urlAnh}" alt="Ảnh gương sáng" class="img-fluid rounded">
                    </div>
                </c:if>
                <div class="col-md-8 d-flex flex-column justify-content-between">
                    <div>
                        <h5 class="fw-bold" style="color: #003399;">
                            <a href="xem-guong-sang-sv?matintuc=${club.maTinTuc}" style="text-decoration: none; color: inherit;">
                                ${club.tieuDeTinTuc}
                            </a>
                        </h5>
                        ...
                        <c:if test="${not empty club.soLanDoc}">
                            <p class="text-muted small">Lượt xem: ${club.soLanDoc}</p>
                        </c:if>
                        <c:if test="${not empty news.tag}">
                            <p class="text-muted small">Tags: ${club.tag}</p>
                        </c:if>
                    </div>
                    <div>
                        <a href="xem-guong-sang-sv?matintuc=${club.maTinTuc}" class="fw-bold text-primary">Xem thêm</a>
                    </div>
                </div>
            </div>
        </c:forEach>
